...
<div class="product-card card {{$product['current_stock']==0?'stock-card':''}}"
    ...
    <div class="card-header inline_product clickable ali" style="cursor: pointer;max-height: 100%;min-height: 75px">
        @if($product->discount > 0)
        ...
        <div class="d-flex align-items-center justify-content-center d-block" >
            <a href="{{route('product',$product->slug)}}">
                <img src="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$product['thumbnail']}}"
                     onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
                     style="width: 80%;height: 80%; object-fit:cover: !important">
            </a>
        </div>
    </div>
